...
pub(crate) struct Guard(Option<JoinHandle<()>>);

pub(crate) fn initialize(plain: bool, no_animation: bool) -> Guard {
...
mod prompt_signals {
    ...
    pub(crate) struct Guard(Vec<(libc::c_int, libc::sigaction)>);
...
pub(crate) fn prompt(context: &str, label: &str, kind: PromptKind) -> Result<Option<String>> {
    pause_progress();
