...
fn destination_readiness(
    ...
    let next_command = match status {
        DestinationReadinessStatus::Ready => repository_root.and_then(|root| {
            ...
            let repository = shell::shell_quoted_path(root)?;
            Some(format!(
                "riftri worktree add {destination} --detach HEAD --repository {repository}"
            ))
        }),
        DestinationReadinessStatus::NeedsActivation => repository_root
            .and_then(shell::shell_quoted_path)
            .map(|repository| format!("riftri enable {repository}"))
